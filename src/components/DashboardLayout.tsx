...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            {/* Sidebar */}
            <aside
                className={`fixed inset-y-0 left-0 z-50 transition-all duration-300 ${sidebarOpen ? "w-64" : "w-20"
                ...
                <nav className="mt-6 px-3 space-y-1">
                    {navItems.map((item) => {
                            ...
                            return (
                                <div key={item.label}>
                                    <button
                                        ...
                                        className={`w-full flex items-center gap-3 px-3 py-3 rounded-xl transition-all duration-200 group ${isChildActive
                                            ? "bg-white text-blue-700 shadow-sm border border-blue-100"
                                            : "text-slate-600 hover:text-blue-700 hover:bg-blue-100/50"
                                            }`}
                                    >
                                        <span className={isChildActive ? "text-blue-600" : "group-hover:text-blue-600"}>{item.icon}</span>
                                        {sidebarOpen && (
                                            <>
                                            ...
                                            {item.children.map((child) => {
                                                ...
                                                return (
                                                    <Link
                                                        key={child.href}
                                                        href={child.href}
                                                        className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-all duration-200 text-sm ${isChildItemActive
                        ...
                        return (
                            ...
                            >
                                <span className={isActive ? "text-blue-600" : "group-hover:text-blue-600"}>{item.icon}</span>
