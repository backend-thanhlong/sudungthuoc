                    ...
                    {rows.map((item, index) => (
                        ...
                        >
                            ...
                            <td className="p-3">
                                ...
                                {item.hamLuong && <p className="text-xs text-gray-500">{item.hamLuong}</p>}
                            </td>
                            ...
                            <td className="p-3 text-right font-mono text-gray-700">{formatNumber(item.demandAvg)}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
            {rows.length === 0 && (
                <p className="text-center text-gray-400 py-8">Không có thuốc hết hàng cuối kỳ trong bộ lọc hiện tại</p>
            )}
        </div>
    );
}
...
function StockoutForecastTable({ rows }: { rows: StockoutForecastRow[] }) {
    return (
        <div className="overflow-x-auto max-h-[350px] overflow-y-auto">
                ...
                <tbody>
                    {rows.map((item, index) => (
                        ...
                        >
                            ...
                            <td className="p-3 text-gray-600">{item.hoatChat}</td>
                            <td className="p-3 text-right font-mono text-gray-700">{formatNumber(item.currentTonCuoi)}</td>
                            <td className="p-3 text-right font-mono text-gray-700">{formatNumber(item.demandAvg)}</td>
            ...
            {rows.length === 0 && (
                <p className="text-center text-gray-400 py-8">Không có thuốc nào có độ phủ tồn kho dưới 3 tháng</p>
            )}
        </div>
    );
}
    ...
    return (
        <div className="overflow-x-auto max-h-60 overflow-y-auto">
            <table className="w-full text-xs">
                ...
                <tbody>
                    {rows.map((item, index) => (
                        <tr key={`${item.facility}-${item.drugName}-${index}`} className="border-b border-gray-50 hover:bg-slate-50">
                            <td className="p-2 text-gray-500">{index + 1}</td>
                            <td className="p-2 text-gray-700">{item.facility}</td>
                            <td className="p-2">
                                <p className="font-medium text-gray-800">{item.drugName}</p>
                                {item.hamLuong && <p className="text-[11px] text-gray-500">{item.hamLuong}</p>}
                            </td>
                            <td className="p-2 text-gray-600">{item.hoatChat}</td>
                            <td className="p-2 text-right font-mono text-slate-700 font-semibold">{formatNumber(item.tonCuoi)}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
            {rows.length === 0 && (
                <p className="text-center text-gray-400 py-6">Không có thuốc tồn kho không phát sinh nhu cầu trong cửa sổ đang chọn</p>
            )}
        </div>
    );
}
        ...
        <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-lg p-5 border border-gray-100">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        <h3 className="font-semibold text-gray-800">Thiết lập cảnh báo cung ứng</h3>
            ...
            <div className="bg-white rounded-xl shadow-lg p-5 border border-gray-100">
                <div className="flex items-center gap-2 mb-1">
                    ...
                    <h3 className="font-semibold text-gray-800">Đã hết hàng cuối kỳ</h3>
                </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-5 border border-gray-100">
                <div className="flex items-center gap-2 mb-1">
                    ...
                    <h3 className="font-semibold text-gray-800">Nguy cơ đứt gãy</h3>
                </div>
                <p className="text-xs text-gray-500 mb-4 ml-9">
            ...
            <div className="bg-white rounded-xl shadow-lg p-5 border border-gray-100">
                <h3 className="font-semibold text-gray-800 mb-1">Ma trận Nhu cầu/Độ phủ tồn kho</h3>
                ...
                <div className="flex flex-wrap gap-2 text-xs mb-4">
                    {Object.entries(SCATTER_SEVERITY_META).map(([severity, meta]) => (
                        <span
                            key={severity}
                            className="inline-flex items-center gap-2 rounded-full border border-gray-200 bg-white px-3 py-1 text-gray-600"
                        >
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        <ScatterChart margin={{ top: 20, right: 30, left: 10, bottom: 20 }}>
                            ...
                            <XAxis
                                ...
                                label={{ value: "Nhu cầu bình quân / kỳ", position: "bottom", offset: 0, style: { fontSize: 12, fill: "#94a3b8" } }}
                            />
                            <YAxis
                                type="number"
                                ...
                                label={{ value: "Số tháng đủ dùng", angle: -90, position: "insideLeft", style: { fontSize: 12, fill: "#94a3b8" } }}
                            />
                            <ZAxis range={[30, 60]} />
                                ...
                                content={({ payload }) => {
                                    ...
                                    return (
                                        <div className="bg-white rounded-lg border border-gray-200 p-3 shadow-xl text-sm">
                                            <p className="font-semibold text-gray-800">{point.drugName}</p>
                                            <p className="text-gray-500">{point.facility}</p>
                                            <p className="text-slate-500">{point.hoatChat}</p>
                                            {point.hamLuong && <p className="text-slate-400">{point.hamLuong}</p>}
                ...
                {data.scatterData.length === 0 && (
                    <p className="text-center text-gray-400 py-6">Không có thuốc nào phát sinh nhu cầu để hiển thị trên ma trận</p>
                )}

                <div className="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-4">
                    <div className="flex items-center justify-between gap-3 mb-3">
                        <div>
                            <h4 className="font-semibold text-slate-800">Tồn kho không có nhu cầu</h4>
            ...
            <div className="bg-white rounded-xl shadow-lg p-5 border border-gray-100">
                ...
                {selectedHoatChat && (
                    <p className="text-xs text-gray-500 mb-4">
                        Đang phân tích hoạt chất: <span className="font-semibold text-gray-700">{selectedHoatChat}</span>
                    </p>
                )}

                {!selectedHoatChat && (
                    <p className="text-center text-gray-400 py-6 text-sm">Nhập hoặc chọn một hoạt chất để hiện gợi ý điều chuyển</p>
                )}
                        ...
                        <div className="border border-emerald-200 rounded-lg overflow-hidden">
                            <div className="bg-emerald-50 px-4 py-2 border-b border-emerald-200">
                                <h4 className="font-semibold text-emerald-800 text-sm flex items-center gap-2">
                                    ...
                                    <span className="text-xs font-normal text-emerald-600">(Độ phủ &gt; 3 tháng)</span>
                                </h4>
                            </div>
                                ...
                                <table className="w-full text-xs">
                                    ...
                                    <tbody>
                                        {transferData.surplus.map((item, index) => (
                                            <tr key={`${item.facility}-${index}`} className="border-t border-gray-50">
                                                ...
                                                <td className="p-2 text-right font-mono text-emerald-600 font-semibold">{formatNumber(item.monthsOfCover)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                                {transferData.surplus.length === 0 && (
                                    <p className="text-center text-gray-400 py-4 text-xs">Không có CSYT nào dư tồn trên 3 tháng</p>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="border border-red-200 rounded-lg overflow-hidden">
                            <div className="bg-red-50 px-4 py-2 border-b border-red-200">
                                <h4 className="font-semibold text-red-800 text-sm flex items-center gap-2">
                                    ...
                                    <span className="text-xs font-normal text-red-600">(Tồn cuối = 0 và vẫn có nhu cầu)</span>
                                </h4>
                            </div>
                            <div className="max-h-60 overflow-y-auto">
                                <table className="w-full text-xs">
                                ...
                                {transferData.shortage.length === 0 && (
                                    <p className="text-center text-gray-400 py-4 text-xs">Không có CSYT nào tồn bằng 0 trong kỳ này</p>
                                )}
                            </div>
                        </div>
                    </div>
